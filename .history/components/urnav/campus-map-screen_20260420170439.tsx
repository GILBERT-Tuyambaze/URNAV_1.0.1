...
import type { ViewState } from '@/lib/map-transform';

export function CampusMapScreen() {
  const [selectedBuildingId, setSelectedBuildingId] = useState<string | null>(null);
  const [selectedRoomId, setSelectedRoomId] = useState<string | null>(null);
  const [currentFloor, setCurrentFloor] = useState(1);
  const [view, setView] = useState<ViewState>({ panX: 0, panY: 0, scale: 0.5, screenW: 800, screenH: 600 });
  const [selectedRoute, setSelectedRoute] = useState<{ points: { mx: number; my: number }[] } | null>(null);

  const selectedBuilding = selectedBuildingId ? ALL_BUILDINGS.find(b => b.id === selectedBuildingId) : undefined;
